...
def exec(connection, input) {
    ...
    if(convert2Roads) {
        ...
        String osmImport = "DROP TABLE MAP_ROADS_speed IF EXISTS;\n" +
                ...
                "CREATE TABLE MAP_ROADS_HGW(ID_WAY BIGINT PRIMARY KEY,HIGHWAY_TYPE varchar(30) ) AS SELECT DISTINCT ID_WAY, VALUE HIGHWAY_TYPE FROM MAP_WAY_TAG WT, MAP_TAG T WHERE WT.ID_TAG = T.ID_TAG AND T.TAG_KEY IN ('highway', 'route');\n" +
